...
class UserController extends Controller
{
      ...
      if (isset ( session('deliverier')->id)) { //exite
        ...
        //if (isset($user->province)) {
            $provincias = DB::table('provincias')
                        ->where('provincias.department_id', 'like', '%'. $user->department . '%')
                        ->get();
        //}
        //if (isset($user->district)) {

            $distritos = DB::table('distritos')
                        ->where('distritos.department_id', 'like', '%'. $user->department . '%')
    ...
    public function UpdateUbication(Request $request, $id){
    ...
    public function UploadPhotos(Request $request){
        ...
        if($request->hasFile('gallery')) {
            ...
            $imagen = $request->file('gallery');
            //dd($imagen);
            foreach($imagen as $imagenes){

                //$name = time().$imagenes->getClientOriginalName();
                $name = $imagenes->getClientOriginalName();
                $imagenes->move(public_path().'/images/',$name);
